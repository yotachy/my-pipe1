...
if (isset($_GET['fg'])) {
    ...
    if ($data && strpos($data, '"fear_and_greed"') !== false) {
        ...
        if (file_exists($cacheFile)) readfile($cacheFile); else echo '{"error":true}';
    }
    exit;
}
...
// 💡 여기서 캐시 타임을 10초 -> 2초로 줄여야만 3초/5초 보상 갱신이 먹힙니다.
...
function getYahooAuth() {
    ...
    curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER => true, CURLOPT_HEADER => true, CURLOPT_TIMEOUT => 5, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_USERAGENT => 'Mozilla/5.0']);
    ...
    curl_setopt_array($ch2, [CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 5, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_USERAGENT => 'Mozilla/5.0', CURLOPT_HTTPHEADER => ["Cookie: $cookie"]]);
...
curl_setopt_array($ch3, [CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 10, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_HTTPHEADER => $headers]);
$data = curl_exec($ch3); curl_close($ch3);

if ($data && strpos($data, '"result"') !== false) {
    @file_put_contents($cacheFile, $data); echo $data;
} else {
    if (file_exists($cacheFile)) readfile($cacheFile); else echo '{"quoteResponse":{"result":[]}}';
}